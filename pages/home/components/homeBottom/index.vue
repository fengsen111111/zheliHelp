	...
	<view class="">
		<view class="mt20">
			<view class="flex col999999 text-center">
				<view class="font-bold" :class="checkIndex==1?'col-black text32':''" @click="handleIndex(1)">
					推荐商品
					<view class="checkItem mx-auto" v-if="checkIndex==1"></view>
				</view>
				<view class="ml100" :class="checkIndex==2?'col-black text32':''" @click="handleIndex(2)">
					团购
					<view class="checkItem mx-auto" v-if="checkIndex==2"></view>
				</view>
				<view class="ml100" :class="checkIndex==3?'col-black text32':''" @click="handleIndex(3)">
					组合套餐
			...
			<view v-else-if="checkIndex==2">
				<view class="flex justify-between mt20">
					<view class="bg-white flex text20 radius8 py6 px10" @click="pickerShow">
						<text>{{columns[0][curIndexs[0]].label}}</text>
						<image src="@/static/home/time.png" class="w28 h28 ml10" mode=""></image>
					</view>
					<view class="bg-white flex text20 radius8 py6 px10" @click="toggle('bottom')">
						<text>全部</text>
